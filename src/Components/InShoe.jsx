...
function InShoe({ cartItems, setCartItems }) {
  const location = useLocation();
  const navigate = useNavigate();
  const product = location.state; // product data passed from Home
  const [selectedSize, setSelectedSize] = useState("7"); // default size
  ...
  // Additional views (optional: back/side images)
  const views = [
  ...
  // Shoe sizes (example: US sizes)
  const sizes = ["6", "7", "8", "9", "10", "11", "12"];

  return (
    <>
      <section className="category-section inshoe-detail">
        <div className="category-row">
          ...
          {/* ✅ Product Info */}
          <div className="product-info inshoe-info">
